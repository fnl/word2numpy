...
# Modified to run with Python 3k by Florian Leitner <flo@fnl.es>
# Additional changes: cleaned code to pass flake8 tests
...
TODO: input vocabulary must be in all lower-case!

OVERVIEW
--------

Module for deep learning via *hierarchical softmax skip-gram* from [1]_.
The training algorithm was originally ported from the C package
https://code.google.com/p/word2vec/ and extended with additional functionality.

**Install Cython with `pip install cython` before to use optimized word2vec training**
(70x speedup [2]_).
...
.. [2] Optimizing word2vec in gensim,
...
blas = lambda name, ndarray: linalg.get_blas_funcs((name,), (ndarray,))[0]
...
def unitvec(vec):
    ...
    """
    if sparse.issparse(vec):  # convert scipy.sparse to standard numpy array
        vec = vec.tocsr()
...
try:
    ...
    logger.exception('pyx failed to compile')
    ...
    def train_sentence(model, sentence, alpha, work=None):
        ...
        for pos, word in enumerate(sentence):
            ...
            for pos2, word2 in enumerate(
                ...
                l1 += np.dot(ga, l2a)  # learn input -> hidden
...
class Vocab(object):
    ...
    def __str__(self):
        vals = ['%s:%r' % (key, self.__dict__[key]) for key in sorted(self.__dict__)
...
class Word2Vec(object):
    ...
    The model can be stored/loaded via the `save()` and `load()` methods, or stored/loaded in a
    format compatible with the original word2vec implementation via `save_word2vec_format()` and
    ...
    def load(cls, fname):
        """
        Load a previously saved Word2Vec object from file (also see `save`).
        """
    ...
    def save(self, fname):
        """
        Save the Word2Vec object to file via pickling (also see `load`).
        """
    ...
    def __init__(self, sentences=None, size=100, alpha=0.025, window=5, min_count=5,
                 seed=1, workers=2, min_alpha=0.0001):
        """
        Initialize the model from an iterable of `sentences`. Each sentence is a
        list of words (strings) that will be used for training.
        ...
        `workers` = use this many worker threads to train the model (faster training with
        multicore machines)
        """
    ...
    def create_binary_tree(self):
        ...
        for i in range(len(self.vocab) - 1):
            ...
            heapq.heappush(heap, Vocab(count=min1.count + min2.count, index=i + len(self.vocab),
        ...
        if heap:
            ...
            while stack:
                ...
                if node.index < len(self.vocab):
                    ...
                    points = np.array(list(points) + [node.index - len(self.vocab)],
                                      dtype=np.uint32)
            ...
            logger.info("built Huffman tree with a maximum node depth of %i", max_depth)
    ...
    def build_vocab(self, sentences):
        ...
        sentence_no, vocab = -1, {}
        total_words = 0
        prog_msg = "\rPROGRESS: {: >15} words  {: >9} vocabulary"
        for sentence_no, sentence in enumerate(sentences):
            if sentence_no % 1000 == 0:
                print(prog_msg.format(total_words, len(vocab)), end='', file=sys.stderr)
        ...
        print(prog_msg.format(total_words, len(vocab)), file=sys.stderr)
        logger.info("collected %i distinct words from a corpus of %i sentences",
                    len(vocab), sentence_no + 1)
        ...
        for word, v in vocab.items():
            if v.count >= self.min_count:
                ...
                self.vocab[word] = v
        logger.info("%i distinct words after removing those with count < %s",
    ...
    def train(self, sentences, total_words=None, word_count=0, chunksize=100):
        """
        Update the model's neural weights from a sequence of sentences (can be a once-only
        generator stream).
        Each sentence must be a list of strings.
        """
        if FAST_VERSION < 0:
            import warnings
            warnings.warn("Cython compilation failed, training will be slow. "
                          "Do you have Cython installed? `pip install cython`")
        ...
        word_count, total_words = [word_count], total_words or sum(
                v.count for v in self.vocab.values())
        ...
        # (=number of words trained so far, log reports...)
        ...
        # convert input strings to Vocab objects (or None for OOV words),
        ...
        logger.info("reached the end of input; waiting to finish %i outstanding jobs",
                    jobs.qsize())
    ...
    def reset_weights(self):
        ...
        self.syn0 += (np.random.rand(len(self.vocab), self.layer1_size) - 0.5) / self.layer1_size
        self.syn0norm = None
    ...
    def load_word2vec_format(cls, fname, binary=False):
        ...
        with open(fname) as fin:
            ...
            result.syn0 = zeros_aligned((vocab_size, layer1_size), dtype=REAL)
            if binary:
                ...
                for line_no in range(vocab_size):
                    ...
                    result.syn0[line_no] = np.fromstring(fin.read(binary_len), dtype=REAL)
                    fin.read(1)  # newline
            else:
    ...
    def most_similar(self, positive=[], negative=[], topn=10):
        ...
        """
        self.init_sims()
        logger.debug("find %d similar workds using +%s -%s", topn, positive, negative)
        ...
        mean = unitvec(np.array(mean).mean(axis=0)).astype(REAL)

        logger.info("distances = sim_%i · mean_%i", len(self.syn0norm), len(mean))
        dists = np.dot(self.syn0norm, mean)
        if not topn:
            return dists
        best = reversed(np.argsort(dists))[:topn + len(all_words)]
    ...
    def doesnt_match(self, words):
        ...
        unit = lambda word: unitvec(self.syn0[self.vocab[word].index])
        vectors = np.vstack(unit(w) for w in words).astype(REAL)
        mean = unitvec(vectors.mean(axis=0)).astype(REAL)
    ...
    def accuracy(self, questions, restrict_vocab=30000):
        ...
        """
        ok_vocab = dict(sorted(
            self.vocab.items(), key=lambda item: -item[1].count)[:restrict_vocab])
        ...
        logger.info("reading questions from %s", questions)
        ...
        for line_no, line in enumerate(open(questions)):
            ...
            if line.startswith(': '):
                ...
                if not section:
                    msg = "missing section header before line {:d} in {}"
                    raise ValueError(msg.format(line_no, questions))
                try:
                    # TODO: assumes vocabulary preprocessing uses lowercase, too...
                    a, b, c, expected = [word.lower() for word in line.split()]
                except:
                    logger.info("skipping invalid line %i in %s", line_no, questions)
                if a not in ok_vocab or \
                        b not in ok_vocab or \
                        c not in ok_vocab or \
                        expected not in ok_vocab:
                ...
                ranking = self.most_similar(positive=[b, c], negative=[a], topn=False)
                for index in reversed(np.argsort(ranking)):
                    if index in ok_index and index not in ignore:
                        predicted = self.index2word[index]
                        if predicted != expected:
                            logger.info("%s: expected %s, predicted %s",
    ...
    def __str__(self):
        return "Word2Vec(vocab=%s, size=%s, alpha=%s)" % (
                len(self.index2word), self.layer1_size, self.alpha)
    ...
    def __iter__(self):
        ...
        with open(self.fname) as fin:
            while True:
                ...
                if text == rest:  # EOF
                    sentence.extend(rest.split())  # return the last chunk of words, too
                    # (may be shorter/longer)
                    if sentence:
                        yield sentence
                    break
                last_token = text.rfind(' ')  # the last token may have been split in two...
                # keep it for the next iteration
                if last_token >= 0:
                    words, rest = (text[:last_token].split(), text[last_token:].strip())
                else:
                    words, rest = ([], text)
...
# Example: ./word2vec.py text8 word2vec/questions-words.txt resultfile
if __name__ == "__main__":
    logging.basicConfig(format='%(asctime)s %(levelname)s : %(message)s',
                        level=logging.INFO)
    logging.info("arguments %s", sys.argv[1:])
    logging.info("%susing cython optimizations", "not " if FAST_VERSION == -1 else "")
